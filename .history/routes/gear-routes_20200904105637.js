...
const { Build, Gear } = require("../models/buildModel")
...
router.post("/", auth, async (req, res) => {
    ...
    if (!build)
        return res
            .status(400)
            .json({ msg: "No build found with this ID." });

    try {
        const gearToCreate = { piece, set, image, weight, trait, glyph, bonus_item_1, bonus_item_2, bonus_item_3, bonus_item_4, bonus_item_5 } = req.body
        if (!piece)
            return res
                .status(400)
                .json({ msg: "You can't save a gear with no piece associated." });

        build.gears.push(gearToCreate)
        await build.save()
        res.json(build.gears[build.gears.length - 1]);
...
router.put("/:id", auth, async (req, res) => {
    const gearToUpdate = { piece, set, image, weight, trait, glyph, bonus_item_1, bonus_item_2, bonus_item_3, bonus_item_4, bonus_item_5 } = req.body
