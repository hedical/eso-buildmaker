...
export default function ImgMediaCard(props) {
    ...
    const shareBuild = (build) => {
        setSelectedBuild(build._id)
        let copyText = `${window.location.pathname}/build/${build._id}`
        copyText.select();
    ...
    return (
        ...
        <div className="card text-white col-3 m-2">
            ...
            <div className="row mx-auto">
                ...
                <Chip className="m-1" label="Edit" size="small" color="secondary" />
                <Chip className="m-1" label="Share" size="small" color="secondary" onClick={() => { shareBuild(props.build) }} />
                <Chip className="m-1" label="Delete" size="small" color="secondary" onClick={() => { confirmAction(props.build) }} />
                <Chip className="m-1" label="View details" size="small" color="secondary" onClick={() => { selectBuild(props.build) }} />
            </div>
