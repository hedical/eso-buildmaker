        ...
        try {
            await axios
                ...
                .then(async (gearList) => {
                    ...
                    let newGearList = [...gearList]
                    setFinalGear({ ...finalGear, piece: selectedPiece })
                })
                .then(() => {
                    console.log(...newGearList);
                    for (let i = 0; i < newGearList.length; i++) {

                        if (finalGear.piece == newGearList[i].piece) {
                            console.log("A corresponding gear in this build has been found");
                            await setFinalGear({
                                gearId: newGearList[i]._id,
                                trait: newGearList[i].trait,
                                set: newGearList[i].set,
                                piece: newGearList[i].piece,
                                weight: newGearList[i].weight,
                                glyph: newGearList[i].glyph,
                            })


                        } else if (newGearList.piece !== finalGear.piece || newGearList.length === 0) {
