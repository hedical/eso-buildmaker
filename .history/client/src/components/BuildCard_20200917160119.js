...
export default function ImgMediaCard(props) {
    ...
    function statBorder() {
        if (props.build.stats) {
            let mag = props.build.stats.magicka
            let stam = props.build.stats.stamina
            let health = props.build.stats.health

            if (mag > stam > health || mag > health > stam) {
                return "blue"
            }
            else if (stam > mag > health || stam > health > mag) {
                return "green"
            }
            else if (health > mag > health || health > stam > mag) {
                return "red"
            }
        }
    ...
    return (
        ...
        <div className="card text-white col-3 m-2" style={borderStyle}>
