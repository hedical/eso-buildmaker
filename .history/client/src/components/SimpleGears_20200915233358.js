                    ...
                    {finalGear.set ?
                        < Autocomplete
                            ...
                            onChange={(event, newValue) => {
                                ...
                                setSet(newValue)
                                // finalGear.set.name ? setValue(finalGear.set.name) : setValue({set:newValue});
                            }}
                            // inputValue={inputValue}
                            value={value.set ? value.set : value}
                            // onInputChange={(event, newInputValue) => {
                            //     finalGear.set.name ? setValue(finalGear.set.name) : setValue({set:newInputValue});
                            // }}
                            disableClearable
                            options={props.sets}
                            getOptionLabel={(option) => typeof option === 'string' && option.name ? option.name : value.set}
                            style={{ width: 200 }}
                            renderInput={(params) =>
                                <TextField {...params}
                                    label="Search set"
